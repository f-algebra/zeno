...
lookupTerm :: MonadState ParserState m => String -> m (Maybe ETerm)
lookupTerm name = gets (Map.lookup name . parsing_terms)

lookupTypeVar :: MonadState ParserState m => String -> m (Maybe TypeVar)
lookupTypeVar name = gets (Map.lookup name . parsing_tvars) 

lookupDataType :: MonadState ParserState m => String -> m (Maybe EDataType)
lookupDataType name = gets (Map.lookup name . parsing_dtypes)
